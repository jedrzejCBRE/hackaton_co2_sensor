        $(document).ready(function () {
            ...
            $('.time_input').on('change', function() {
                let raw_id = event.target.id.split("/")
                $.ajax({
                    url: "{% url 'update-labour-time-ajax' %}",
                    headers: {
                ...
                .done(function(data) {
                    console.log(data);
                    $('#updated_time_popup').modal('show');
                    setTimeout(function(){
                        $('#updated_time_popup').modal('hide');
                    },2000);
                })
            });
            $("#search_SFG_code").keyup(function(){
                // Retrieve the input field text and reset the count to zero
                var filter = $(this).val(), count = 0;
                $("#no-count").text('');
                // Loop through the comment list
                $("#table_body tr td").each(function(){
